    ...
    <style>
        ...
        * {
        ...
        body {
            ...
            align-items: center;
            padding: 110px 15px 90px 15px; /* پدینگ پایین برای جلوگیری از رفتن محتوا زیر بنر فیکس پایین */
            position: relative;
            overflow-x: hidden;
        }
        ...
        /* ریسپانسیو و استانداردسازی فوق‌العاده ابعاد تصاویر فیلم‌ها */
        .movie-poster {
            width: 100%;
            border-radius: 16px;
        ...
        /* تعیین ابعاد دقیق بر اساس نوع دستگاه */
        @media (max-width: 679px) {
            .movie-poster { height: 400px; } /* ابعاد ایده آل موبایل */
        }
        @media (min-width: 680px) and (max-width: 991px) {
            .movie-poster { height: 380px; } /* ابعاد ایده آل تبلت */
        }
        @media (min-width: 992px) {
            .movie-poster { height: 420px; } /* ابعاد ایده آل کامپیوتر */
        }
        ...
        /* --- استایل بنر تبلیغاتی کاملاً شناور و فیکس در پایین صفحه (موبایل و تبلت) --- */
        ...
        .floating-banner-tablet { display: none; }

        @media (min-width: 680px) and (max-width: 991px) {
            .floating-banner-mobile { display: none !important; }
            .floating-banner-tablet { display: block !important; }
        }
        @media (min-width: 992px) {
            .floating-bottom-banner { display: none !important; } /* غیرفعال در دسکتاپ */
        }


        /* --- استایل پاپ‌آپ جدید عمودی لوکس (مخصوص موبایل و تبلت) --- */
        .gateway-overlay {
            position: fixed;
        ...
        /* جدول/کادر مستطیل عمودی برای متون و انیمیشن شمارنده */
        .gateway-text-table {
            width: 100%;
            ...
            margin-bottom: 25px; /* ایجاد فاصله تا تبلیغ خارج کادر */
        }

        /* تایمر انیمیشنی معکوس ۶ ثانیه‌ای */
        ...
        /* هولدر خالص تبلیغات زیر کادر بدون قاب اضافی */
        .gateway-popup-ad-holder {
            width: 100%;
            display: flex;
            justify-content: center;
            align-items: center;
...
<body>
        ...
        <div class="movies-container">
            ...
            <div class="movie-block-wrapper">
                <div class="movie-premium-card" id="movie-The-Marked-Woman">
                    ...
                    <button onclick="handleDownloadAction('https://t.me/AVA_Movies_Animes/9')" class="premium-download-btn">DOWNLOAD NOW</button>
                </div>
            </div>
            ...
            <div class="movie-block-wrapper">
                <div class="movie-premium-card" id="Equilibrium">
                    ...
                    <button onclick="handleDownloadAction('https://t.me/AVA_Movies_Animes/14')" class="premium-download-btn">DOWNLOAD NOW</button>
                </div>
            </div>
    ...
    <div class="floating-bottom-banner" id="live-floating-banner-container">
        ...
        <div class="floating-banner-tablet">
            <script>
    ...
    <div id="downloadGatewayModal" class="gateway-overlay">
        <div class="gateway-mobile-wrapper">
            
            <div class="gateway-text-table">
                <div id="gatewayCountdownTimer" class="gateway-timer-display">6</div>
            ...
            <div class="gateway-popup-ad-holder" id="popup-native-ad-box">
                <script async="async" data-cfasync="false" src="https://pl29741961.effectivecpmnetwork.com/46fa53bab184d6979bf4dbeee413d25f/invoke.js"></script>
                <div id="container-46fa53bab184d6979bf4dbeee413d25f"></div>
            </div>
    ...
    <script>
        ...
        const moviesDatabase = [
            { id: "movie-The-Marked-Woman", title: "The Marked Woman", thumb: "https://images.justwatch.com/poster/344584556/s718/the-marked-woman.jpg" },
            { id: "Equilibrium", title: "Equilibrium", thumb: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTzVWU77H_SgJSuhupxH1-c7pHcKN_cmjVXyBdn_2TjO4oc8SL8-T86nVo&s=10" }
        ];
        ...
        searchInput.addEventListener('input', function() {
            ...
            if (query === '') { resultsDropdown.style.display = 'none'; return; }

            const filteredMovies = moviesDatabase.filter(movie => movie.title.toLowerCase().includes(query));

            if (filteredMovies.length > 0) {
                filteredMovies.forEach(movie => {
                    const tab = document.createElement('div');
                    tab.className = 'result-tab-item';
                    tab.innerHTML = `<img src="${movie.thumb}" class="result-thumb"><span class="result-title">${movie.title}</span>`;
                    tab.addEventListener('click', function() {
                        const targetCard = document.getElementById(movie.id);
                        if (targetCard) {
                            targetCard.scrollIntoView({ behavior: 'smooth', block: 'start' });
                            targetCard.style.borderColor = 'var(--neon-blue)';
                            setTimeout(() => { targetCard.style.borderColor = 'rgba(255, 255, 255, 0.06)'; }, 2000);
                        }
                        searchInput.value = '';
        ...
        // --- مدیریت کلیک دکمه دانلود بر اساس نوع دستگاه ---
        function handleDownloadAction(destinationUrl) {
            // اگر عرض صفحه بزرگتر یا مساوی 992 بود (لپ‌تاپ و کامپیوتر) -> مستقیم هدایت شود
            if (window.innerWidth >= 992) {
                window.location.href = destinationUrl;
            } else {
                // اگر موبایل یا تبلت بود -> پاپ‌آپ ۶ ثانیه‌ای لوکس ظاهر شود
                triggerMobileGateway(destinationUrl);
            }
        }

        // پاپ‌آپ اختصاصی موبایل
        function triggerMobileGateway(destinationUrl) {
            const modal = document.getElementById('downloadGatewayModal');
            const timerDisplay = document.getElementById('gatewayCountdownTimer');
            
            let timeLeft = 6; // ۶ ثانیه کامل
            timerDisplay.innerText = timeLeft;
            modal.style.display = 'flex';

            // رفرش زنده اسکریپت نیتیو داخل پاپ‌آپ موبایل جهت نمایش قطعی
            const adBox = document.getElementById('popup-native-ad-box');
            adBox.innerHTML = '';
            const sNode = document.createElement('script');
            sNode.async = true; sNode.setAttribute('data-cfasync', 'false');
            sNode.src = 'https://pl29741961.effectivecpmnetwork.com/46fa53bab184d6979bf4dbeee413d25f/invoke.js';
            const dNode = document.createElement('div');
            dNode.id = 'container-46fa53bab184d6979bf4dbeee413d25f';
            adBox.appendChild(sNode);
            adBox.appendChild(dNode);
        ...
        // --- سیستم رفرش اتوماتیک بنر شناور پایین صفحه (هر ۱۹ ثانیه یک‌بار) مخصوص موبایل/تبلت ---
        setInterval(() => {
            const bannerContainer = document.getElementById('live-floating-banner-container');
            if (bannerContainer && window.innerWidth < 992) {
                const currentWidth = window.innerWidth;
                bannerContainer.innerHTML = ''; // ریست کامل بنر قبلی
                
                if (currentWidth < 680) {
                    // رفرش بنر موبایل ۳۲۰ در ۵۰
                    const wrap = document.createElement('div');
                    wrap.className = 'floating-banner-mobile';
                    ...
                    // رفرش بنر تبلت ۴۶۸ در ۶۰
                    const wrap = document.createElement('div');
                    wrap.className = 'floating-banner-tablet';
        ...
        }, 19000); // دقیقاً هر ۱۹ ثانیه یک‌بار رفرش مجدد بنر فیکس پایین صفحه جهت درآمدزایی نهایی

        // --- سیستم رفرش تبلیغ سایدبار دسکتاپ (هر ۵۰ ثانیه یک‌بار) ---
        setInterval(() => {
            const sidebar = document.getElementById('desktop-sidebar-container');
            if (sidebar && window.innerWidth >= 992) {
                sidebar.innerHTML = ''; 
                const adScript = document.createElement('script');
                adScript.async = true; adScript.setAttribute('data-cfasync', 'false');
